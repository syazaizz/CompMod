...
class Particle(object):
    ...
    * kinetic energy
    ...
    def pot_energy(p1,p2):
        return -(10**(-9))*p1.mass*p2.mass/np.linalg.norm(Particle.Vect_Sep(p1,p2))                       # change unit G from m^3kg^-1s^-2 to km^3kg^-1s^-2 

    #gravitational force between two particles
    @staticmethod
    def force(p1,p2):
        return (-(10**(-9))*p1.mass*p2.mass/(np.linalg.norm(Particle.Vect_Sep(p1,p2)))**3)*Particle.Vect_Sep(p1,p2)
